...
npy_norm_folder = r"E:\THESIS\ADNI_data\ADNI1_Annual_2_Yr_3T_306_WORK\INTEREST_NPY_DATA\Normalized_NPY\{}_normNPY\\"

clahe_folder = r"E:\THESIS\ADNI_data\ADNI1_Annual_2_Yr_3T_306_WORK\INTEREST_NPY_DATA\CLAHE_NPY\CLAHE_{}npy\\"
...
def modify_image_orientation(case, srcfol, files:list):
    ...
    """
    os.chdir(srcfol.format(case))
    for file in os.listdir():
        fname, fext = os.path.splitext(file)
        serial = int(re.findall('\d+', fname)[0])
...
def convert_npy_to_img(srcfol: str, tarfol: str, case: str, tup:dict):
    """[Convert 3d npy data files to 2d images]

    Args:
        srcfol (str): [folder locations of the case types]
        tarfol (str): [folder location of the case types]
        case (str): ['AD'/"CN'/'MCI']
        tup (dict): [dict of tuples holding the image range casewise]
    """
    os.chdir(srcfol.format(case))
    for file in os.listdir():
        fname, fext = os.path.splitext(file)
        ...
        for d in data:
            imgloc = f'{tarfol}{case}\\{case}-{fname}\\'
            if not os.path.exists(imgloc):
                Path(imgloc).mkdir(parents=True, exist_ok=True)
            imgfile = imgloc + f'{case}-{serial}_img{x}.png'
            cv2.imwrite(imgfile, d)
            x += 1
    return


def get_img_names(dir:str):
...
def convert_img_to_npy(srcfol: str, tarfol: str, case:str, tuples: dict):
    """convert 2d image files into a 3d npy file

    Args:
        srcfol ([str]): [source location of casewise folders]
        tarfold ([str]): [target location of casewise folders]
        case ([str]): ['AD'/'CN'/'MCI']
        tuples (dict): [a dictionary of tuples for selected range]
    """
    for serial in selected_files[case]:
        os.chdir(srcfol.format(case, case, serial))
        folder = os.getcwd()
        start, finish = tuples[serial]
        images = []
        for slices in range(start, finish + 1):
            fname = f'\\{case}-{serial}_img{slices}.jpg'
            file = folder + fname
            img = mpimg.imread(file)
            print(f'{fname} loaded')
            images.append(img)
        data = np.array(images)
        npy_save = tarfol.format(case) + f'{case}-Data{serial}.npy'
...
def calc_CLAHE(data):   # contrast limit histogram equalization
    images = []
    for i in range(data.shape[0]):
        ...
        images.append(CLAHE_npy)
    CLAHE_data = np.array(images)
    return CLAHE_data


def generate_CLAHE_data():
    for case in ['AD', 'CN', 'MCI']:
        folder = npy_norm_folder.format(case)
        os.chdir(folder)
        print('Inside',case)
        for file in os.listdir():
            fname, fext = os.path.splitext(file)
            ...
            data = np.load(file, allow_pickle=True)
            print('loaded')
            CLAHE_data = calc_CLAHE(data)
            print(file,'CLAHE obtained')
            save_as = clahe_folder.format(case)+fname+"_clahe"+fext
            np.save(save_as, CLAHE_data)
            print(fname+"_clahe"+fext,'saved')
    return
