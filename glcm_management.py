...
clahe_glcm = r"E:\THESIS\ADNI_data\ADNI1_Annual_2_Yr_3T_306_WORK\INTEREST_NPY_DATA\CLAHE_NPY\CLAHE_GLCM\CLAHE_glcm_{}\\"
...
def save_glcm_feats(case, serial, features):
    ...
    np.save((save_as.format(case) + "{}-clahe-asm{}".format(case, serial)), asms)
    print(f'{case}-{serial}-clahe asm', 'saved')

    np.save((save_as.format(case) + "{}-clahe-contrast{}".format(case, serial)), con)
    print(f'{case}-{serial}-clahe contrast', 'saved')

    np.save((save_as.format(case) + "{}-clahe-correlation{}".format(case, serial)), corr)
    print(f'{case}-{serial}-clahe correlation', 'saved')

    np.save((save_as.format(case) + "{}-clahe-dissimlarity{}".format(case, serial)), diss)
    print(f'{case}-{serial}-clahe dissimilarity', 'saved')

    np.save((save_as.format(case) + "{}-clahe-energy{}".format(case, serial)), en)
    print(f'{case}-{serial}-clahe energy', 'saved')

    np.save((save_as.format(case) + "{}-clahe-homogeneity{}".format(case, serial)), homo)
    print(f'{case}-{serial}-clahe homogeneity', 'saved')
    return
...
def calc_glcm(case):
    ...
    for case in cases:
        os.chdir(clahe_folder.format(case))
...
def perform_data_generation(cases=['AD', 'CN', 'MCI']):
    ...
    for c in cases:
        ...
        for file in os.listdir():
            ...
            #np.save(imputed_data_fold.format(c) + f"{fname}-imp{fext}", interpolated_data)
            np.save(clahe_glcm.format(c) + f"{fname}-imp{fext}", interpolated_data)
...
def merge_glcm():
    ...
    for case in ['AD', 'CN', 'MCI']:
        ...
        for serial in cases[case]:
            ...
            for feat in feats:
                print(f'{feat} -', end=' ')
                form = f"{case}-clahe-{feat}{serial}-imp.npy"
                #data = np.load(imputed_data_fold.format(case) + form, allow_pickle=True)
                data = np.load(clahe_glcm.format(case) + form ,  allow_pickle=True)
                data = data.flatten()
                d.append(data)
    ...
    np.save(intrst_data_fol+"clahe_glcm_54.npy", case_data_array)
...
if __name__ == "__main__":
    #calc_glcm(['AD','CN','MCI'])
    limit = 0
    #limit = get_max_slice(r"E:\THESIS\ADNI_data\ADNI1_Annual_2_Yr_3T_306_WORK\INTEREST_NPY_DATA\CLAHE_NPY\CLAHE_GLCM\CLAHE_glcm_{}\\")
    limit = 69
